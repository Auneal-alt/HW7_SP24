...
from PyQt5.QtWidgets import QWidget, QApplication, QGroupBox, QVBoxLayout, QLabel, QGridLayout, QLineEdit, QComboBox
...
class main_window(QWidget, Ui__frm_StateCalculator):
    def __init__(self):
        ...
        self.setUnits()
        self.show()
        self.createStateContainer("State 1")
        self.createStateContainer("State 2")

    def createStateContainer(self, state_name):
        # Create a container for a state within the group box
        stateGroupBox = QGroupBox(state_name)

        # Add the state container to the specified properties layout
        self.verticalLayout_2.addWidget(stateGroupBox)

        # Create a layout for the state container
        stateContainerLayout = QGridLayout(stateGroupBox)

        # Add widgets to the state container
        lbl_Property1 = QLabel("Property 1")
        lbl_Property2 = QLabel("Property 2")
        le_Property1 = QLineEdit()
        le_Property2 = QLineEdit()
        cmb_Property1 = QComboBox()
        cmb_Property2 = QComboBox()
        lbl_Property1_Units = QLabel("Units")
        lbl_Property2_Units = QLabel("Units")

        # Add widgets to the layout
        stateContainerLayout.addWidget(lbl_Property1, 0, 0)
        stateContainerLayout.addWidget(le_Property1, 0, 1)
        stateContainerLayout.addWidget(cmb_Property1, 0, 2)
        stateContainerLayout.addWidget(lbl_Property1_Units, 0, 3)
        stateContainerLayout.addWidget(lbl_Property2, 1, 0)
        stateContainerLayout.addWidget(le_Property2, 1, 1)
        stateContainerLayout.addWidget(cmb_Property2, 1, 2)
        stateContainerLayout.addWidget(lbl_Property2_Units, 1, 3)

        # Add more widgets as needed

        # Connect signals for property indexes
        cmb_Property1.currentIndexChanged.connect(self.setUnits)
        cmb_Property2.currentIndexChanged.connect(self.setUnits)

        return stateGroupBox  # Return the created group box
